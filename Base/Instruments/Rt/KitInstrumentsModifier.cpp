    ...
    const util::Identifiable::UUID& sdUuid, int sdVoiceIdx) noexcept
{
   return getInstrument(instrumentUuid).and_then(
      [&,this](auto instrumentIt) -> Void {
         return KitInstrumentModifier(*instrumentIt).createNewVoiceInKitInstrument(rMDContainer, sdUuid, sdVoiceIdx);
      });
}
    ...
    const util::Identifiable::UUID& sdUuid, int sdVoiceIdx) noexcept
{
   return getInstrument(instrumentUuid).and_then(
      [&,this](auto instrumentIt) -> Void {
         return KitInstrumentModifier(*instrumentIt).addComponentToKitInstrumentVoice(rMDContainer, voiceIdx, sdUuid, sdVoiceIdx);
      });
}
