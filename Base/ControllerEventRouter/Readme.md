...
map as a whole
struct DrumKit{
   util::Identifiable::UUID trackUuid; // or instrumentUuid
};

1/b
// with note = ALL_NOTES it plays it chromatically, otherwise just triggers it with a predefined pitch 
map to a specific drumkit voice
struct DrumKit{
   util::Identifiable::UUID trackUuid; // or instrumentUuid
   int voice;
};
...
1/c
map to a specific drumkit parameter
struct DrumKit{
   util::Identifiable::UUID trackUuid; // or instrumentUuid
   int voice;
   int componentIdx;
...
map to a md voice
struct DrumKit{
   util::Identifiable::UUID mdUuid;
   int voice;
};

3/b
map to a md parameter
struct DrumKit{
   util::Identifiable::UUID mdUuid;
   int voice;
   Parameter p;
