...
{
   mpark::visit(
       util::overload{[this, &eventExt, &value](const WidgetCoord& widgetCoord) {
                         const auto destIter = m_map.find(eventExt);
                         if (destIter != m_map.end())
                         {
                            //handlePressReleaseDirect();
                            const auto mdIter = m_rMusicDeviceContainer.find(destIter->second.uuid);
                            if(mdIter != m_rMusicDeviceContainer.end() && mdIter->second->soundHandler)
                            {
                                mpark::visit(
                                    util::overload{
                                        [&mdIter, &destIter, &eventExt, &value](const EventDestination::Note& note){
                                            if(value.value > 0)
                                            {
                                                mdIter->second->soundHandler->noteOn(destIter->second.voiceIdx, note.value, value.value);
                                            }
                                            else
                                            {
                                                mdIter->second->soundHandler->noteOff(destIter->second.voiceIdx, note.value, -value.value);
                                            }
                                        },
                      ...
                      [this, &eventExt](const Note& note) {
                         ...
                         else
                         {
                            ...
                            melodicEvent.event.id.widgetCoord.emplace<Note>(
                                Note{-1});
...
void EventRouter::handleContinousValueType(const EventExt& event,
...
void EventRouter::handleRelativeValueType(const EventExt& event,
