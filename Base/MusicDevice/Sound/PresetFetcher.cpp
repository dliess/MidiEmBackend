...
void PresetFetcher::fetchPresets()
{
  ...
  */
        ...
        engineIdx < m_pDescription->soundSection->engines.size(); ++engineIdx)
   {
      ...
      {
         if (voice.engineId == engineIdx &&
             m_pDescription->soundSection->engines[engineIdx].presets &&
             (m_pDescription->soundSection->parameterDumpRequest ||
              voice.parameterDumpRequest))
         {
            for (int i = 0; i < m_pDescription->soundSection->engines[engineIdx]
                                    .presets->numberOfPresets;
                 ++i)
            {
               m_pMidiOut->send(midi::Message<midi::ProgramChange>(
                   voice.midiChannel - 1, i));
               const uint8_t TODO_midiVoiceOffset = 0;
               ParameterDumpRequest(*m_pMidiOut, *m_pDescription->soundSection,
